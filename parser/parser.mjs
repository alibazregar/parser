...
class HTMLParser {
                  ...
                  if (attributeConfig) {
                    if (attributeConfig.To) {
                      if (
                        ...
                        (typeof finalAttributes[attributeConfig.To] ==
                          "string" ||
                          typeof finalAttributes[attributeConfig.To] ==
                            "object")
                      ) {
                  ...
                  if (
                    ...
                    items.forEach((bindItems) => {
                      ...
                      if (splitedBindItems.length == 3) {
                        ...
                        value.push({ Value: splitedBindItems[0].slice(1, -1) });
                      }
                    });
   ...
   * @returns {object}
   */
  static async parseBodyAndChangeToWebP(htmlContent, parserConfig = "parserConfig") {
    try {
      const __filename = fileURLToPath(import.meta.url);
      const __dirname = path.dirname(__filename);
      const parser = new HTMLParser("", "/configs");
      parser.html = this.parseBodyAndChangeToWebP(htmlContent);
      const parserPath = path.join(__dirname, parserConfig + ".json");
      await parser.getTheParserConfig(parserPath);
      const tokens = parser.parse();
      const result = await parser.processTags(tokens);
...
HTMLParser.parse("html", "final", "parserConfig");